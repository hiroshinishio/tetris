...
class Game_Manager(QMainWindow):
    ...
    def keyPressEvent(self, event):
        ...
        if self.isPaused:
            return
        elif key == Qt.Key.Left:
            BOARD_DATA.moveLeft()
        elif key == Qt.Key.Right:
            BOARD_DATA.moveRight()
        elif (key == Qt.Key.Up and self.mode == 'keyboard') or (key == Qt.Key.Space and self.mode == 'gamepad'):
        ...
        elif (key == Qt.Key.Space and self.mode == 'keyboard') or (key == Qt.Key.Up and self.mode == 'gamepad'):
            ## テトリミノを一番下まで落とす
            removedlines, dropdownlines = BOARD_DATA.dropDown()
            # 消去ライン数と落下数によりスコア計算
            self.UpdateScore(removedlines, dropdownlines)
